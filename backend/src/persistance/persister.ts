...
export class PlanetScalePersister implements Persister{
    ...
    async getMessages(roomId:string) {
        ...
        const data = (await this.db.select().from(encryptedMessages).where(eq(encryptedMessages.roomId, roomId)).orderBy(desc(encryptedMessages.id)).limit(500)).reverse() //needs the most recent 500 in reverse order. Seem to need subqueries to accomplish this, which I assume count as row reads so I'll just do it here
        //TODO: pagination
